...
export const ReactionProc = (
    { user_id, channel_id, message_id, emoji: { id } }: Reaction,
    add: boolean,
) => {
...
export const SetMarks = async (emojis: Types.Emoji[]) => {
    ...
    for (const emoji of emojis) emojiMap.set(emoji.id, emoji);

    for (const channel_id of Object.getOwnPropertyNames(marks)) {
        const messages = await Actions.Channel.GetMessages(channel_id, {
            limit: 10,
        }).catch(Logger.Error);
        ...
        for (const message_id of Object.getOwnPropertyNames(channel)) {
            const { reactions } =
                messages.find(({ id }) => id == message_id) ?? {};
            ...
            for (const eid of Object.getOwnPropertyNames(message)) {
                ...
                await Actions.Reaction.Add(
                    channel_id,
                    message_id,
                    Tools.Format.Reaction(emojiMap.get(eid)),
                ).catch(Logger.Error);
            }
        }
    }
